"""End-to-end tests for StonksLogger with PyTorch Lightning.

Follows the same testing pattern as Lightning's test_wandb.py and
test_comet.py: functional tests with a BoringModel-style DummyModel,
pytest fixtures, and full Trainer integration.
"""
...
from stonks.store import create_connection, get_metric_keys, list_runs

# ---------------------------------------------------------------------------
# Helpers (mirrors Lightning's BoringModel)
# ---------------------------------------------------------------------------


class DummyModel(pl.LightningModule):
    """Minimal model for e2e tests, equivalent to Lightning's BoringModel."""
...
def test_fit_logs_train_and_val_metrics(stonks_logger, dummy_data, trainer_kwargs, db_path):
    """fit() produces both train and val metric keys in the database."""
...
def test_fit_logs_hyperparameters(stonks_logger, dummy_data, trainer_kwargs, db_path):
    """Hyperparameters from save_hyperparameters() are stored in the run config."""
    trainer = pl.Trainer(logger=stonks_logger, **trainer_kwargs)
    trainer.fit(DummyModel(in_features=8, lr=0.01), datamodule=DummyDataModule(in_features=8))
...
def test_fit_finalizes_run_as_completed(stonks_logger, dummy_data, trainer_kwargs, db_path):
    """Successful fit() marks the run as completed."""
    trainer = pl.Trainer(logger=stonks_logger, **trainer_kwargs)
    trainer.fit(DummyModel(), datamodule=dummy_data)
...
def test_validate_before_fit(stonks_logger, dummy_data, trainer_kwargs, db_path):
    """validate() before fit() triggers log_hyperparams without crashing."""
    trainer = pl.Trainer(logger=stonks_logger, **trainer_kwargs)
    trainer.validate(DummyModel(), datamodule=dummy_data)
...
def test_multiple_loggers(db_path, dummy_data, trainer_kwargs):
    """StonksLogger works alongside other loggers (like WandB + TensorBoard)."""
...
def test_max_steps_mode(stonks_logger, dummy_data, trainer_kwargs, db_path):
...
# ---------------------------------------------------------------------------
# OmegaConf serialization (regression tests for fix/omegaconf-serialization)
# ---------------------------------------------------------------------------


def test_omegaconf_listconfig_in_hparams(db_path, dummy_data, trainer_kwargs):
    ...
    trainer.fit(_Model(), datamodule=dummy_data)
...
def test_omegaconf_dictconfig_in_hparams(db_path, dummy_data, trainer_kwargs):
...
def test_omegaconf_nested_config_in_hparams(db_path, dummy_data, trainer_kwargs):
