...
class TestStonksLogger:
    def test_lazy_initialization(self, db_path):
        ...
        logger = StonksLogger("test-exp", save_dir=str(db_path))
    ...
    def test_name_property(self, db_path):
        ...
        logger = StonksLogger("my-experiment", save_dir=str(db_path))
    ...
    def test_experiment_reuse(self, db_path):
        ...
        logger1 = StonksLogger("shared-exp", save_dir=str(db_path))
        logger1.log_metrics({"loss": 1.0}, step=0)
        logger1.finalize("success")

        logger2 = StonksLogger("shared-exp", save_dir=str(db_path))
